...
def pred():
    ...
    st.header('Enter title for Recommendation')
    title = st.selectbox('Type / Select from the dropbox', data['title'].values)
    run_recommendation = st.number_input('Select the no. of recommendations', min_value=1, max_value=50, value=10)
    results = get_recommendation(title,run_recommendation)
